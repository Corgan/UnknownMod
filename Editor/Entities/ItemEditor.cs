...
namespace UnknownMod.Editor
{
    ...
    /// </summary>
    public class ItemEditor : ModProjectEditorBase<ItemDef>
    {
        // ── Base class overrides ─────────────────────────────────
        protected override string TypeLabel => "Item";
        protected override string FolderName => "items";
        protected override string NewIdSuffix => "_new_item";

        public override string SelectedId
        {
            get => Parent.SelectedItemId;
            set => Parent.SelectedItemId = value;
        }

        protected override Dictionary<string, ItemDef> GetNewDict(ModProject proj) => proj.Items;
        protected override Dictionary<string, ItemDef> GetPatchDict(ModProject proj) => proj.ItemPatches;

        protected override ItemDef CreateDefault(string id, ModProject proj)
            => new ItemDef { Id = id, Name = "New Item" };

        protected override string GetDisplayName(ItemDef def) => def.Name;

        protected override ItemDef SnapshotBaseEntity(string id)
        {
            var item = DataHelper.GetItem(id);
            return item != null ? DataHelper.SnapshotItem(item) : null;
        }
        ...
        public ItemEditor(ModEditor parent) : base(parent) { }

        // ═══════════════════════════════════════════════════════════════
        //  ALL SECTIONS
        // ═══════════════════════════════════════════════════════════════

        protected override void DrawAllSections(ItemDef d, ModProject proj)
        {
            // ── Live Stat Preview ────────────────────────────────
        ...
        public static string BuildItemDescription(ItemDef d)
        {
            ...
            { sb.Append($"{sep}+<color=#44cc88>{d.AuracurseBonusValue1}</color> {EditorFields.Capitalize(d.AuracurseBonus1)} bonus"); sep = "\n"; }
            if (!string.IsNullOrEmpty(d.AuracurseBonus2) && d.AuracurseBonusValue2 != 0)
            { sb.Append($"{sep}+<color=#44cc88>{d.AuracurseBonusValue2}</color> {EditorFields.Capitalize(d.AuracurseBonus2)} bonus"); sep = "\n"; }

            // ── AC immunities ────────────────────────────────────
            if (!string.IsNullOrEmpty(d.AuracurseImmune1))
            { sb.Append($"{sep}<color=#88ccff>Immune</color> to {EditorFields.Capitalize(d.AuracurseImmune1)}"); sep = "\n"; }
            if (!string.IsNullOrEmpty(d.AuracurseImmune2))
            { sb.Append($"{sep}<color=#88ccff>Immune</color> to {EditorFields.Capitalize(d.AuracurseImmune2)}"); sep = "\n"; }
            ...
            if (d.Activation != Enums.EventActivation.None)
            {
                ...
                { sb.Append($"{sep}  Apply <color=#cc6644>{d.AuracurseGainValue1} {EditorFields.Capitalize(d.AuracurseGain1)}</color>"); sep = "\n"; }
                if (!string.IsNullOrEmpty(d.AuracurseGainSelf1) && d.AuracurseGainSelfValue1 != 0)
                { sb.Append($"{sep}  Gain <color=#44cc88>{d.AuracurseGainSelfValue1} {EditorFields.Capitalize(d.AuracurseGainSelf1)}</color>"); sep = "\n"; }
