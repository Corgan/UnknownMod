...
namespace UnknownMod.Editor
{
    ...
    public class CardEditor : ModProjectEditorBase<CardDef>
    {
        ...
        protected override string NewIdSuffix => "_new_card";

        public override string SelectedId
        {
            get => Parent.SelectedCardId;
            set => Parent.SelectedCardId = value;
        }
        ...
        protected override CardDef CreateDefault(string id, ModProject proj)
            => new CardDef { Id = id, Name = "New Card" };
        ...
        private static void AppendAC(StringBuilder sb, ref string sep, string acId, int charges, string verb, string color)
        {
            ...
            {
                sb.Append($"{sep}{verb} <color={color}>{charges}</color> <color={color}>{EditorFields.Capitalize(acId)}</color>");
                sep = "\n";
            }
        }
