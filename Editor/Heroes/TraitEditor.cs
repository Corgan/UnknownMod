...
namespace UnknownMod.Editor
{
    ...
    public class TraitEditor : ModProjectEditorBase<TraitDef>
    {
        ...
        protected override EntityPicker.Mode? PickerMode => EntityPicker.Mode.Trait;
        ...
        protected override void DrawAllSections(TraitDef d, ModProject proj)
        {
            ...
            if (EditorFields.Section("Cards", ref _secCards))
            {
                ...
                d.TraitCard = EditorFields.IdDropdown("Trait Card", d.TraitCard, cardIds, "tr_card", pickerMode: EntityPicker.Mode.Card);
                d.TraitCardForAllHeroes = EditorFields.IdDropdown("Card (All Heroes)", d.TraitCardForAllHeroes, cardIds, "tr_cardall", pickerMode: EntityPicker.Mode.Card);
            }
            ...
            if (EditorFields.Section("AC Immunity", ref _secImmunity))
            {
                ...
                d.AuracurseImmune1 = EditorFields.IdDropdown("Immune 1", d.AuracurseImmune1, acIds, "tr_imm1", pickerMode: EntityPicker.Mode.AuraCurse);
                d.AuracurseImmune2 = EditorFields.IdDropdown("Immune 2", d.AuracurseImmune2, acIds, "tr_imm2", pickerMode: EntityPicker.Mode.AuraCurse);
                d.AuracurseImmune3 = EditorFields.IdDropdown("Immune 3", d.AuracurseImmune3, acIds, "tr_imm3", pickerMode: EntityPicker.Mode.AuraCurse);
            }
            ...
            {
                var acIds = EditorFields.CachedIds("ac", DataHelper.GetAllAuraCurseIds);
                d.AuracurseBonus1 = EditorFields.IdDropdown("AC 1", d.AuracurseBonus1, acIds, "tr_acb1", pickerMode: EntityPicker.Mode.AuraCurse);
                d.AuracurseBonusValue1 = EditorFields.IntField("Value 1", d.AuracurseBonusValue1);
                GUILayout.Space(4);
                d.AuracurseBonus2 = EditorFields.IdDropdown("AC 2", d.AuracurseBonus2, acIds, "tr_acb2", pickerMode: EntityPicker.Mode.AuraCurse);
                d.AuracurseBonusValue2 = EditorFields.IntField("Value 2", d.AuracurseBonusValue2);
                GUILayout.Space(4);
                d.AuracurseBonus3 = EditorFields.IdDropdown("AC 3", d.AuracurseBonus3, acIds, "tr_acb3", pickerMode: EntityPicker.Mode.AuraCurse);
            ...
            if (EditorFields.Section("Misc", ref _secMisc))
            {
                ...
                d.HideTimesPerTurnText = EditorFields.Toggle("Hide Times/Turn Text", d.HideTimesPerTurnText);

                GUILayout.Space(4);
                GUILayout.Label($"<color=#aaa>Key Notes ({d.KeyNotes?.Count ?? 0}):</color>", EditorStyles.RichLabel);
                if (d.KeyNotes == null) d.KeyNotes = new List<string>();
                for (int i = 0; i < d.KeyNotes.Count; i++)
                {
                    GUILayout.BeginHorizontal();
                    d.KeyNotes[i] = EditorFields.TextField($"Note {i + 1}", d.KeyNotes[i]);
                    if (GUILayout.Button("\u00d7", GUILayout.Width(22)))
                    {
                        d.KeyNotes.RemoveAt(i);
                        GUI.changed = true;
                ...
                if (GUILayout.Button("+ Key Note", EditorStyles.MiniButton, GUILayout.Width(80)))
                {
                    d.KeyNotes.Add("");
                    GUI.changed = true;
                }
