...
namespace UnknownMod.Editor
{
    ...
    public class CardPlayerPackEditor
    {
        ...
        private void DrawPlayerPanel(ModProject proj)
        {
            ...
            string prevPackId = d.PackId;
            DrawPlayerSections(d, proj);

            if (GUI.changed)
            {
                if (d.PackId != prevPackId && !string.IsNullOrEmpty(d.PackId))
                {
                    var dict = isPatch ? proj.CardPlayerPackPatches : proj.CardPlayerPacks;
                    dict.Remove(prevPackId);
                    dict[d.PackId] = d;
                    ModProjectLoader.DeleteEntity(proj, "cardplayerpacks", prevPackId, isPatch);
                    SelectedPlayerPackId = d.PackId;
                }
        ...
        private void DrawPlayerOverrideBrowser(ModProject proj)
        {
            ...
            {
                if (shown >= 50) break;
                if (!string.IsNullOrEmpty(filt) && !id.ToLower().Contains(filt)) continue;
        ...
        private void DrawPlayerSections(CardPlayerPackDef d, ModProject proj)
        {
            ...
            if (EditorFields.Section("Card Slots (0-3)", ref _secSlotsP))
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    d.Slots[i].CardId = EditorFields.IdDropdown($"  Card ID", d.Slots[i].CardId, cardIds, $"cpp_slot{i}", pickerMode: EntityPicker.Mode.Card);
        ...
        private void DrawPairsPanel(ModProject proj)
        {
            ...
            string prevPackId = d.PackId;
            DrawPairsSections(d, proj);

            if (GUI.changed)
            {
                if (d.PackId != prevPackId && !string.IsNullOrEmpty(d.PackId))
                {
                    var dict = isPatch ? proj.CardPlayerPairsPackPatches : proj.CardPlayerPairsPacks;
                    dict.Remove(prevPackId);
                    dict[d.PackId] = d;
                    ModProjectLoader.DeleteEntity(proj, "cardplayerpairspacks", prevPackId, isPatch);
                    SelectedPairsPackId = d.PackId;
                }
        ...
        private void DrawPairsSections(CardPlayerPairsPackDef d, ModProject proj)
        {
            var cardIds = EditorFields.BuildCardIdList(proj);
            ...
            if (EditorFields.Section("Cards (0-5)", ref _secCardsPP))
            {
                ...
                for (int i = 0; i < 6; i++)
                    d.CardIds[i] = EditorFields.IdDropdown($"Card {i}", d.CardIds[i], cardIds, $"cppp_card{i}", pickerMode: EntityPicker.Mode.Card);
            }
        }
    }
}
