...
namespace UnknownMod.Editor
{
    ...
    public class LootEditor : ModProjectEditorBase<LootDef>
    {
        ...
        protected override EntityPicker.Mode? PickerMode => EntityPicker.Mode.Loot;
        ...
        protected override void DrawAllSections(LootDef d, ModProject proj)
        {
            ...
            // ── Shady Model ──────────────────────────────────────
            if (EditorFields.Section("Shady Model", ref _secShady))
            {
                d.ShadyScaleX = EditorFields.FloatField("Scale X", d.ShadyScaleX);
                d.ShadyScaleY = EditorFields.FloatField("Scale Y", d.ShadyScaleY);
                d.ShadyOffsetX = EditorFields.FloatField("Offset X", d.ShadyOffsetX);
                d.ShadyOffsetY = EditorFields.FloatField("Offset Y", d.ShadyOffsetY);
                var lootIds = EditorFields.CachedIds("loot", DataHelper.GetAllLootIds);
                d.ShadyModelSource = EditorFields.IdDropdown("Model Source", d.ShadyModelSource, lootIds, "loot_shadysrc", pickerMode: EntityPicker.Mode.Loot);
            }
            ...
            if (EditorFields.Section($"Loot Items ({d.Items.Count})", ref _secItems))
            {
                ...
                for (int i = 0; i < d.Items.Count; i++)
                {
                    ...
                    // Card reference
                    item.CardId = EditorFields.IdDropdown("Card ID", item.CardId, cardIds, $"loot_card_{i}", pickerMode: EntityPicker.Mode.Card);
