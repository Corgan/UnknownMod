...
namespace UnknownMod.Core
{
    public static partial class ZoneEditingService
    {
        private static void CacheZoneFromTransform(string zoneId, Transform zoneT)
        {
            ...
            // from the zone hierarchy, excluding Nodes and Roads subtrees.
            {
                ...
                foreach (Transform child in zoneT)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Container with children  scan children for sprites too
                ...
                //  Clone non-sprite visual GameObjects 
            ...
            if (roadsT != null)
            {
                // Compute offset: Roads-parent-local  Nodes-parent-local
                ...
                foreach (Transform roadT in roadsT)
                {
                    ...
                    // Transform from road GO local space  Nodes parent space:
            ...
            //  Dump visual hierarchy (non-Node/Road elements) 
        ...
        private static void DumpZoneVisualHierarchy(string zoneId, Transform zoneT)
        {
            ...
            foreach (Transform child in zoneT)
            {
                ...
                {
                    sb.AppendLine($"  [{child.name}] ({child.childCount} children)  skipped");
                    continue;
                }
        ...
        /// Safe to call multiple times  no-ops if already loaded/loading.
        /// Called from ModEditor.ToggleEditMode when the dev panel opens.
        /// </summary>
        public static void PreloadScenes()
        {
            // Map scene  zone backgrounds, node positions, roads, visual layers
            ...
            // Combat scene  combat background prefabs
            EntityPreviewRenderer.EnsureBgCache();

            // Event scene  reply prefab, event layout template
        ...
        /// <summary>Get all cached layer sprites for a zone (name  Sprite).</summary>
