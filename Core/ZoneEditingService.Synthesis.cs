...
namespace UnknownMod.Core
{
    public static partial class ZoneEditingService
    {
        ...
        //  ZONE PATCH SYNTHESIS â€” build a full ZoneDef from base + patch
        // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

        // Cache: zoneId â†’ { nodeId â†’ (posX, posY) }
        private static readonly Dictionary<string, Dictionary<string, Vector2>> _positionCache = new(StringComparer.OrdinalIgnoreCase);
        // Cache: zoneId â†’ { roadKey â†’ all road points[] (in Nodes-parent-relative coords) }
        ...
        // Cache: base-game zone visual layer sprites (Name â†’ Sprite) for rendering
        ...
        // Cache: container offsets â€” zoneId â†’ (nodesOffset, roadsOffset)
        ...
        // Cache: base-game mapPiece data per node: zoneId â†’ { nodeId â†’ List<MapPieceDef> }
        ...
        // Cache: mapPiece sprites: zoneId â†’ { spriteName â†’ Sprite }
        ...
        public static ZoneDef SynthesizeZoneDef(ZonePatchDef patch)
        {
            ...
            if (!_positionCache.ContainsKey(zoneId))
            {
                ...
                if (!CacheBaseZoneData(zoneId))
                {
                    ...
                    // without caching failure â€” caller will retry next frame.
                    if (_sceneLoadFailed)
                    {
            ...
            // â”€â”€ Populate base-game nodes from Globals + position cache â”€â”€
            ...
            // â”€â”€ Merge in patch nodes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            foreach (var kvp in patch.Nodes)
                synth.Nodes[kvp.Key] = kvp.Value;

            // â”€â”€ Populate base-game roads from cache â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            // â”€â”€ Merge in patch roads â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            // â”€â”€ Events: snapshot from Globals â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                foreach (var nd in synth.Nodes.Values)
                {
                    foreach (var eid in nd.EventIds)
                    {
                        if (!string.IsNullOrEmpty(eid))
                            referencedEventIds.Add(eid.ToLower());
                    }
                }
            ...
            // â”€â”€ Combats: snapshot from Globals â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                foreach (var nd in synth.Nodes.Values)
                {
                    foreach (var cid in nd.CombatIds)
                    {
                        if (!string.IsNullOrEmpty(cid))
                            referencedCombatIds.Add(cid.Replace(" ", "").ToLower());
                    }
                }
            ...
            // â”€â”€ Merge in patch additions (override base) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        private static bool CacheBaseZoneData(string zoneId)
        {
            ...
            // â”€â”€ Async additive scene load: load the Map scene to access
